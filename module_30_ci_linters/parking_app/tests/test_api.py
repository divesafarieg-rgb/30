...
class TestClientsAPI:
    ...
    @pytest.mark.parametrize(
        "client_data,expected_status,expected_fields",
        [
            ({"name": "Анна", "surname": "Сидорова"}, 201, ["id", "name", "surname"]),
            (
                {
                    "name": "Мария",
                    "surname": "Иванова",
...
class TestParkingOperations:
    ...
    def test_enter_parking_no_places(self, client, test_data):
        new_parking = {"address": "Тестовая парковка с 1 местом", "count_places": 1, "opened": True}
        ...
        enter1_data = {"client_id": client1_id, "parking_id": parking_id}
        enter1_response = client.post("/client_parkings", json=enter1_data)
        assert enter1_response.status_code == 201

        enter2_data = {"client_id": client2_id, "parking_id": parking_id}
    ...
    def test_enter_nonexistent_client(self, client, test_data):
        enter_data = {"client_id": 999, "parking_id": test_data["parking_open"]}
    ...
    def test_exit_without_card(self, client, test_data):
        new_client_data = {"name": "Без", "surname": "Карты", "car_number": "TEST01"}
        create_response = client.post("/clients", json=new_client_data)
        assert create_response.status_code == 201
        client_without_card_id = create_response.get_json()["id"]

        enter_data = {"client_id": client_without_card_id, "parking_id": test_data["parking_open"]}
        enter_response = client.post("/client_parkings", json=enter_data)
        assert enter_response.status_code == 201

        exit_data = {"client_id": client_without_card_id, "parking_id": test_data["parking_open"]}
        exit_response = client.delete("/client_parkings", json=exit_data)
